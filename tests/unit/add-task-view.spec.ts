...
describe('task-detail-view', () => {
  describe('describeActions', () => {
    it('pending: claim / complete / abandon / edit / delete all visible', () => {
      const a = describeActions(buildDetail({ status: 'pending' }))
      expect(a).toEqual({ claim: true, complete: true, abandon: true, edit: true, delete: true })
    })

    it('claimed: claim hidden, complete / abandon / edit / delete visible', () => {
      const a = describeActions(buildDetail({
        status: 'claimed',
        assignee: { nickname: 'me', avatar: { kind: 'builtin', id: 'person-01' } },
      }))
      expect(a).toEqual({ claim: false, complete: true, abandon: true, edit: true, delete: true })
    })
