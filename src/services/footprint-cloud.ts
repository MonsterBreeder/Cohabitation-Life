...
export function setFootprintCloudRuntimeForTesting(value: CloudRuntime | undefined): void {
  runtimeForTesting = value
  initialized = false
}
export function setFootprintCloudEnvironmentForTesting(value: string | undefined): void {
  environmentForTesting = value
  initialized = false
}
export function setFootprintCloudTimeoutForTesting(value: number): void {
  timeoutMs = value
}
export function resetFootprintCloudForTesting(): void {
  initialized = false
...
export function isFootprintEntry(value: unknown): value is FootprintEntrySummary {
  ...
  return (
    ...
    Number(item.editVersion) >= 1
  )
}

function isHikingTimelineEntry(value: unknown): value is HikingEntrySummary {
  if (!value || typeof value !== 'object') return false
  const item = value as Record<string, unknown>
  return (
    item.entryKind === 'hike' &&
    ...
    (item.place === null || isPlace(item.place)) &&
    (item.mapPoint === null ||
      (typeof item.mapPoint === 'object' &&
        item.mapPoint !== null &&
        typeof (item.mapPoint as Record<string, unknown>).latitude === 'number' &&
        typeof (item.mapPoint as Record<string, unknown>).longitude === 'number')) &&
    typeof item.memory === 'string' &&
...
export function isFootprintTimelineEntry(value: unknown): value is FootprintTimelineEntry {
  return isFootprintEntry(value) || isHikingTimelineEntry(value)
}
...
function isPlaceSummary(value: unknown): value is FootprintPlaceSummary {
  ...
  return (
    typeof item.placeKey === 'string' &&
    isPlace(item.place) &&
    Number.isInteger(item.visitCount) &&
    Number(item.visitCount) >= 1 &&
    isFootprintEntry(item.latestEntry)
  )
}
...
function isHomeSummary(value: unknown): value is FootprintHomeSummary {
  ...
  const item = value as Record<string, unknown>
  return (
    Number.isInteger(item.placeCount) &&
    Number(item.placeCount) >= 0 &&
    (item.latestEntry === null || isFootprintEntry(item.latestEntry))
  )
}
...
function isFailure(value: unknown): value is FootprintFailure {
  ...
  return (
    ...
    failureStatuses.has(item.status) &&
...
async function call(action: string, input: Record<string, unknown> = {}): Promise<unknown> {
  ...
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(() => reject(new FootprintCloudError('TIMEOUT', '足迹服务响应超时')), timeoutMs)
  })
  try {
    const response = await Promise.race([
      runtime().callFunction({ name: 'footprint', data: { action, ...input } }),
      timeout,
    ])
...
export async function getFootprintOverviewInCloud(): Promise<FootprintOverview | FootprintFailure> {
  ...
  if (
    item.status !== 'LOADED' ||
    !isHomeSummary(item.summary) ||
    !Array.isArray(item.places) ||
    !item.places.every(isPlaceSummary) ||
    !Array.isArray(item.entries) ||
    !item.entries.every(isFootprintEntry) ||
    typeof item.showPreJoinHistoryNotice !== 'boolean' ||
    (item.entriesCursor !== null && typeof item.entriesCursor !== 'string') ||
    (item.placesCursor !== null && typeof item.placesCursor !== 'string')
  ) {
  ...
  return {
    ...
    entriesCursor: item.entriesCursor,
    placesCursor: item.placesCursor,
    showPreJoinHistoryNotice: item.showPreJoinHistoryNotice,
  }
}

/** 首页不下载地图和时间列表；三个读请求独立失败，互不拖累。 */
export async function getFootprintSummaryInCloud(): Promise<
  { summary: FootprintHomeSummary; showPreJoinHistoryNotice: boolean } | FootprintFailure
> {
  ...
  if (
    ...
    throw new FootprintCloudError('INVALID_RESPONSE', '足迹摘要格式错误')
  return { summary: item.summary, showPreJoinHistoryNotice: item.showPreJoinHistoryNotice }
}

export async function listFootprintEntriesInCloud(input: {
  cursor?: string | null
  placeKey?: string | null
}): Promise<{ entries: FootprintTimelineEntry[]; cursor: string | null } | FootprintFailure> {
  const raw = await call('listEntries', { ...input, responseVersion: 'hiking-v1' })
  if (isFailure(raw)) return raw
  const item = raw as Record<string, unknown>
  if (
    !item ||
    item.status !== 'LISTED' ||
    !Array.isArray(item.entries) ||
    !item.entries.every(isFootprintTimelineEntry) ||
    (item.cursor !== null && typeof item.cursor !== 'string')
  )
    throw new FootprintCloudError('INVALID_RESPONSE', '足迹列表格式错误')
  return { entries: item.entries, cursor: item.cursor as string | null }
}

export async function listFootprintPlacesInCloud(input: {
  cursor?: string | null
}): Promise<{ places: FootprintPlaceSummary[]; cursor: string | null } | FootprintFailure> {
  ...
  if (
    ...
    throw new FootprintCloudError('INVALID_RESPONSE', '足迹地点格式错误')
  return { places: item.places, cursor: item.cursor as string | null }
}

export async function getFootprintEntryInCloud(
  entryId: string,
): Promise<FootprintEntryDetail | FootprintFailure> {
  ...
  if (
    !item ||
    item.status !== 'LOADED' ||
    !isFootprintEntry(detail) ||
    !Array.isArray(detail.photos) ||
    !detail.photos.every(isFootprintPhoto) ||
    !detail.creator ||
    typeof detail.creator !== 'object' ||
    !Number.isInteger(detail.samePlaceCount)
  )
    throw new FootprintCloudError('INVALID_RESPONSE', '足迹详情格式错误')
  return detail as unknown as FootprintEntryDetail
}

export async function createFootprintEntryInCloud(input: {
...
}): Promise<FootprintEntrySummary | FootprintFailure> {
  const raw = await call('createEntry', input)
  if (isFailure(raw)) return raw
  const item = raw as Record<string, unknown>
  if (!item || item.status !== 'CREATED' || !isFootprintEntry(item.entry))
    throw new FootprintCloudError('INVALID_RESPONSE', '新增足迹格式错误')
  return item.entry
}

export async function updateFootprintEntryInCloud(input: {
  expectedHouseholdId?: string
  entryId: string
  ...
  if (isFailure(raw)) return raw
  const item = raw as Record<string, unknown>
  if (!item || item.status !== 'UPDATED' || !isFootprintEntry(item.entry))
    throw new FootprintCloudError('INVALID_RESPONSE', '编辑足迹格式错误')
  return item.entry
}

export async function deleteFootprintEntryInCloud(input: {
  entryId: string
  operationToken: string
}): Promise<{ entryId: string; deletedAt: string } | FootprintFailure> {
  ...
  if (
    ...
    throw new FootprintCloudError('INVALID_RESPONSE', '删除足迹格式错误')
...
export async function prepareFootprintPhotoInCloud(): Promise<FootprintPhotoReservation> {
  ...
  if (
    !item ||
    item.status !== 'UPLOAD_READY' ||
    typeof item.resourceId !== 'string' ||
    typeof item.cloudPath !== 'string'
  )
    throw new FootprintCloudError('INVALID_RESPONSE', '照片上传准备失败')
  return { resourceId: item.resourceId, cloudPath: item.cloudPath }
}

export interface FootprintUploadProgress {
  reservation?: FootprintPhotoReservation
  fileID?: string
}

export async function uploadAndReviewFootprintPhoto(
  localPath: string,
  ...
  if (!progress.fileID)
    progress.fileID = (
      await runtime().uploadFile({ cloudPath: prepared.cloudPath, filePath: localPath })
    ).fileID
  ...
  if (
    ...
    throw new FootprintCloudError('INVALID_RESPONSE', '照片审核失败')
...
export async function getFootprintPhotoUrlsInCloud(resourceIds: string[]): Promise<FootprintPhoto[]> {
  ...
  for (let offset = 0; offset < ids.length; offset += 6) {
    ...
    if (
      !item ||
      item.status !== 'URLS_READY' ||
      !Array.isArray(item.photos) ||
      !item.photos.every(isFootprintPhoto)
    )
      throw new FootprintCloudError('INVALID_RESPONSE', '照片地址格式错误')
