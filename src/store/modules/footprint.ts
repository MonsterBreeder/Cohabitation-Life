...
import {
  acknowledgeFootprintHistoryInCloud, createFootprintEntryInCloud, deleteFootprintEntryInCloud,
  getFootprintEntryInCloud, getFootprintSummaryInCloud, getFootprintPhotoUrlsInCloud,
  humaniseFootprintError, listFootprintEntriesInCloud, listFootprintPlacesInCloud, updateFootprintEntryInCloud,
} from '../../services/footprint-cloud'
import type { FootprintEntryDetail, FootprintEntrySummary, FootprintHomeSummary, FootprintPhoto, FootprintPlace, FootprintPlaceSummary } from '../../types/footprint'

interface State {
  householdId: string; contextVersion: number; summary: FootprintHomeSummary | null
  places: FootprintPlaceSummary[]; entries: FootprintEntrySummary[]
  placesCursor: string | null; entriesCursor: string | null; listPlaceKey: string; listVersion: number
  currentDetail: FootprintEntryDetail | null; detailVersion: number
  phase: 'idle' | 'loading' | 'saving' | 'deleting' | 'failed'
  mapError: string | null; listError: string | null; summaryError: string | null
  errorMessage: string | null; errorCode: string | null
  showPreJoinHistoryNotice: boolean; pending: Record<string, boolean>
}

export const useFootprintStore = defineStore('footprint', {
  state: (): State => ({
    householdId: '', contextVersion: 0, summary: null, places: [], entries: [], placesCursor: null, entriesCursor: null,
    listPlaceKey: '', listVersion: 0, currentDetail: null, detailVersion: 0, phase: 'idle', mapError: null,
    listError: null, summaryError: null, errorMessage: null, errorCode: null, showPreJoinHistoryNotice: false, pending: {},
  }),
  actions: {
    ...
    async loadSummary(): Promise<void> {
      ...
      try {
        ...
        this.summary = result.summary; this.showPreJoinHistoryNotice = result.showPreJoinHistoryNotice; this.summaryError = null
      } catch (error) {
        if (version === this.contextVersion) this.summaryError = humaniseFootprintError(error)
      } finally { if (version === this.contextVersion) this.pending.summary = false }
    },
    async loadOverview(_preserveExisting = true): Promise<void> {
      if (!this.householdId || this.pending.overview) return
      const version = this.contextVersion
      this.pending.overview = true; this.phase = 'loading'
      // 三类读取独立保留成功结果，地图失败不影响列表和首页卡片。
      await Promise.all([this.loadSummary(), this.loadEntries(this.listPlaceKey), this.loadPlaces()])
      if (version !== this.contextVersion) return
    ...
    async loadEntries(placeKey = ''): Promise<void> {
      ...
      if (this.listPlaceKey !== placeKey) { this.entries = []; this.entriesCursor = null }
      this.listPlaceKey = placeKey; this.pending.entries = true
      try {
        ...
        if ('status' in result) throw result
        this.entries = result.entries; this.entriesCursor = result.cursor; this.listError = null
      } catch (error) {
        if (version === this.contextVersion && listVersion === this.listVersion) this.listError = humaniseFootprintError(error)
      } finally { if (version === this.contextVersion && listVersion === this.listVersion) this.pending.entries = false }
    },
    async loadMoreEntries(): Promise<void> {
      ...
      try {
        const result = await listFootprintEntriesInCloud({ cursor: this.entriesCursor, placeKey: this.listPlaceKey || null })
        if (version !== this.contextVersion || listVersion !== this.listVersion) return
        if ('status' in result) throw result
        const known = new Set(this.entries.map((entry) => entry.id))
        this.entries.push(...result.entries.filter((entry) => !known.has(entry.id)))
        this.entriesCursor = result.cursor; this.listError = null
      } catch (error) {
        if (version === this.contextVersion && listVersion === this.listVersion) this.listError = humaniseFootprintError(error)
      } finally { if (version === this.contextVersion && listVersion === this.listVersion) this.pending.entries = false }
    },
    async loadPlaces(): Promise<void> {
      ...
      try {
        do {
          ...
          this.places = [...loaded]; this.placesCursor = cursor; this.mapError = null
        } while (cursor)
      } catch (error) {
        if (version === this.contextVersion) this.mapError = humaniseFootprintError(error)
      } finally { if (version === this.contextVersion) this.pending.places = false }
    },
    async loadEntry(entryId: string): Promise<FootprintEntryDetail | null> {
      ...
      this.phase = 'loading'; this.errorMessage = null; this.currentDetail = null
      try {
        ...
        if ('status' in result) throw result
        this.currentDetail = result; this.phase = 'idle'; return result
      } catch (error) {
        if (version === this.contextVersion && detailVersion === this.detailVersion) { this.errorMessage = humaniseFootprintError(error); this.phase = 'failed' }
        return null
      }
    },
    async hydratePhotoUrls(photos: FootprintPhoto[]): Promise<Record<string, string>> {
      ...
      try {
        ...
        if (version !== this.contextVersion) return {}
        return Object.fromEntries(loaded.filter((photo) => photo.url).map((photo) => [photo.resourceId, photo.url as string]))
      } catch { return {} }
    },
    async createEntry(input: { requestId: string; place: FootprintPlace; visitedAt: string; memory: string; photoResourceIds: string[] }): Promise<FootprintEntrySummary | null> {
      if (!this.householdId || this.pending.write) return null
      const version = this.contextVersion
      this.pending.write = true; this.phase = 'saving'; this.errorMessage = null; this.errorCode = null
      try {
        ...
        return null
      } finally { if (version === this.contextVersion) this.pending.write = false }
    },
    async updateEntry(input: { entryId: string; operationToken: string; editVersion: number; place: FootprintPlace; visitedAt: string; memory: string; photoResourceIds: string[] }): Promise<FootprintEntrySummary | null> {
      if (!this.householdId || this.pending.write) return null
      const version = this.contextVersion
      this.pending.write = true; this.phase = 'saving'; this.errorMessage = null; this.errorCode = null
      try {
    ...
    async deleteEntry(entryId: string, operationToken: string): Promise<boolean> {
      ...
      const version = this.contextVersion
      this.pending.write = true; this.phase = 'deleting'; this.errorMessage = null; this.errorCode = null
      try {
        const result = await deleteFootprintEntryInCloud({ entryId, operationToken })
        if (version !== this.contextVersion) return false
        if ('status' in result) throw result
        this.currentDetail = null; this.entries = this.entries.filter((entry) => entry.id !== entryId)
        await this.loadOverview()
