...
function createRepository(db) {
  ...
  return {
    ...
    getSummary: async (householdId) => {
      const grouped = await entries.aggregate().match({ householdId, deletedAt: null }).group({ _id: '$placeKey' }).count('placeCount').end()
      const latest = await entries.where({ householdId, deletedAt: null }).orderBy('visitedAt', 'desc').orderBy('createdAt', 'desc').orderBy('_id', 'desc').limit(1).get()
    ...
    listPlaces: async (householdId, cursor, limit) => {
      ...
      const result = await entries.aggregate().match({ householdId, deletedAt: null, ...(after ? { placeKey: _.gt(after.id) } : {}) }).sort({ visitedAt: -1, createdAt: -1, _id: -1 }).group({
    ...
    createEntryWithOperation: (entry, operation, photos, actor) => db.runTransaction(async (transaction) => {
    ...
    updateEntryVersioned: async (existing, updates, newPhotos, actor, operation) => db.runTransaction(async (transaction) => {
...
module.exports = { createRepository, withoutId, toSummary, encodeCursor, decodeCursor }
