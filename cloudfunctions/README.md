...
“问账本”只允许通过 `ledger-ai` 云函数访问，模型负责理解问题和组织一句简短回答，真实筛选、排序和金额计算由云函数完成。模型只能看到最多 5 条已匹配的展示摘要，不会读取凭证图片、无关账目、家庭编号、成员内部编号或账目编号。

### 1. 云函数与配置

- 上传 `cloudfunctions/ledger-ai`，选择“云端安装依赖”，超时设置为 30 秒。
- 设置环境变量 `LEDGER_AI_MODEL=hy3` 和 `LEDGER_AI_ENABLED=false`。测试全部通过前不得改为 `true`。
- 普通事件云函数使用当前环境身份，不配置 AppSecret、腾讯云永久密钥或第三方模型密钥。
- 上传 `cloudfunctions/cleanup-ledger-ai-requests`，确认每天 03:20 的定时触发器存在。

### 2. 集合、权限与索引

- 创建 `ledgerAiDailyUsage`、`ledgerAiSessions` 两个集合，安全规则全部设为“小程序客户端不可读写”。
- `ledgerAiDailyUsage` 只保存家庭当天已用次数；`ledgerAiSessions` 保存两小时内的重试结果和候选临时引用，不保存问题正文或模型上下文。
- 为 `ledgerAiSessions.expiresAt` 建立升序索引，供清理函数查询。
- 确认 `ledgerEntries` 已有家庭、删除状态和发生时间所需索引；单次最多读取 1000 条，超过时明确失败，不能返回不完整金额。

### 3. 上线前人工验证

1. A、B 两个家庭成员共享每天 10 次，并发提交第 10、11 次时只能成功一个。
2. 第三方账号和被移除成员调用状态、提问或连续追问时均被拒绝。
3. “第一笔/第二笔”只引用当前用户当前页面的候选；删除候选后追问返回“账目已变化”。
4. 模糊找账、简单汇总和统计页分流分别走通，金额与数据库整数分相加结果一致。
5. 检查云函数和模型日志，不得出现问题正文、回答正文、账目备注、金额明细和内部编号。
6. 模拟体验额度不可用，确认问账本停止回答，而记账、筛选、统计和详情仍能使用。
7. 全部通过后才把 `LEDGER_AI_ENABLED` 改为 `true`，并先在少量账号试用。
...
`previewInvite` 公开字段已收窄：

- 邀请人头像在邀请凭证有效时由 `cloud.getTempFileURL` 换取短时 https URL；仅返回受控编号或短时 URL
- 自定义头像换取短时 URL 失败时退化为默认头像，邀请本身仍可继续
- 公开字段禁止包含邀请原文、家庭编号、成员编号、内部用户编号、家庭业务数据
- 邀请预览只读，不消费邀请，不修改邀请状态；协议确认后由 `resolve-login` 重新核验

云函数发布顺序必须为"先云端、后小程序"；旧客户端可继续使用旧字段校验。代码细节见 [首次体验重构 audit 与实际环境验证记录](../docs/plans/2026-09-11-first-use-audit-verification.md)。
