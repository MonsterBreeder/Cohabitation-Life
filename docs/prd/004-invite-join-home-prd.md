...
- R7. 受邀者打开分享后，如尚未开始使用，应先看到同一份邀请的最小摘要，可选择“加入这个家”或“先看看怎么用”；查看摘要和体验都不创建用户或消费邀请。
- R8. 邀请摘要展示邀请方的昵称、头像、家庭名称和“目前 1 位成员”；不展示邀请方账号、家庭事项、账目、足迹或其他不必要资料。点击“加入这个家”并确认协议后，才进入正式加入判断。
...
- 非家庭成员不能通过猜测链接查看家庭资料或邀请状态；只有持有仍然有效、不可猜测邀请凭证的人可以在开始使用前查看 PRD 001 限定的最小邀请摘要。
...
> 本节只补验收清单，不修改上述任何需求。每一项都必须在测试云环境用两个不同的真实微信账号走完；任一项与预期不符时，停止发布并定位修复。

1. **首次邀请加入。** 账号 A 创建家庭 → 准备邀请 → 通过微信分享给账号 B → B 在加入确认页完成确认 → 双方首页都显示两人家庭，家庭成员数和昵称一致。
2. **两边已建单人家庭后的转入。** A 和 B 各自先创建单人家庭 → A 准备邀请并分享给 B → B 在加入确认页选择“确认转入”并完成二次确认 → 双方均进入 A 的家庭，B 的旧单人家庭不再出现；B 自己的昵称和头像保留。
3. **并发确认同一邀请。** 在测试云环境让两个账号在 2 秒内同时提交同一份邀请的确认 → 仅一个账号成功，另一个账号得到“这个家已经满员”的提示，A 和 B 的旧家庭均未受影响。
4. **创建者移除成员。** 双人家庭中由创建者在成员管理页完成两次确认的移除 → 创建者立即回到单人状态 → 被移除者下次打开或刷新产品时进入创建家庭页，并显示“你已不在这个家中”的说明。
5. **被移除者重进。** 被移除者重新创建家庭 → 旧邀请链接再次被打开时显示“邀请已被使用”或“邀请无效”，被移除者不能直接加回原家庭。
6. **非成员访问。** 第三方账号（未加入任何家庭）尝试访问被分享的家庭头像、名称或成员资料：直接打开分享链接、复制链接重新打开、以及通过控制台拉取云端数据，都不应返回真实家庭资料。
7. **邀请过期。** 将测试云环境的邀请过期时间临时调短（如 1 分钟）或等待 24 小时后再次打开原邀请链接，确认展示“邀请已失效”，且没有写入或修改任何家庭资料。
...
按本 PRD 制定的实施计划 [feat: 重构首次了解、功能体验与开始使用流程](../plans/2026-09-11-001-feat-first-use-experience-plan.md) 已完成邀请入口的代码实现，包括：

- 邀请预览只读，只返回邀请人昵称头像、家庭名、当前成员数；自定义头像经云端换取短时 https URL
- 邀请预览按 token + 版本号作废迟到响应，切换邀请 / 失败 / 主动结束都不残留旧摘要
- 邀请终态页只暴露受控提示，按 hasStartedUse / hasHousehold 决定唯一下一步
- 协议确认后由 `resolve-login` 重新核验邀请，预览结果不作为加入依据

代码层面的验收记录见 [首次体验重构 audit 与实际环境验证记录](../plans/2026-09-11-first-use-audit-verification.md)。**待实际环境确认**的事项：

- 微信开发者工具与真机双账号逐场景验收：全新用户预览与体验、体验期间邀请失效、确认加入、已有家庭用户打开其他邀请、重复点击不会产生第二个家庭
- 微信公众平台类目核对（当前后台选项会变化）
