...
在记账页选择系统预设的“餐饮”类目后，显示“早餐 / 午餐 / 晚餐”三个选项。新建账目时按设备当前本地时间自动选中一个选项，用户可以在保存前修改。所选餐次随账目保存，并在账目列表与详情中可见；既有账目保持兼容，不做批量补写，也不因编辑而被自动改成错误餐次。
...
- 自动选中后，用户可以手动改为另外一个餐次；手动选择不会被无关表单操作覆盖。
...
- **R2 自动默认：** 新建餐饮账目按当前本地小时自动选中早餐、午餐或晚餐，用户可修改。
...
- 用户把自动值从午餐改成晚餐后，修改金额、日期和备注仍保持晚餐。
